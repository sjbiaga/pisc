...
programmatically typed as `Scala` code using `ZIO` `UIO`.

Composition: parallel modelled with - `ZIO.collectAllParDiscard`.

Ambient: singly parallelized with - `ZIO.collectAllParDiscard` and `FiberRef`.

[Guarded] Replication: modelled with - `ZIO.collectAllParDiscard` and `lazy val` [or `def`].


Ambients
--------

Recall that using `collectAllParDiscard` in `ZIO` resorts to fibers. There isn't really any
distinction between fibers ran in parallel, so not between ambients, were it not
for the possibility to have per-fiber copies of the same `FiberRef`. And so, in
fact, ambients are not differentiated at runtime (running in parallel) except
that the "_ether_" used for communication is resolved starting with an `FiberRef`.

An `FiberRef` is a parameter to each agent (so including "`Main`") invocation, and is
thus available to be altered when "launching" ambients in parallel. The value of
these `FiberRef`s are `UUID`s. Were it not for the capability to "`open`" ambients,
...
`FiberRef` is associated with an `UUID`. When an ambient is "`open`"ed, there
exist already fibers running in parallel, each with its `FiberRef`: those
...
What happens given an `FiberRef` is that, to retrieve its node, the keys of the
tree `Map` are searched the `UUID` of the `FiberRef`, for membership to a key-`Set`.
...
If there are more `App`s' with agents that depend one to another, pass the `--interactive` option and all source files:
    ...
    ./examples $ ma -zio --interactive ex1.scala ex2.scala
