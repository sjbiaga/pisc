...
The root project folder `akka` contains three files: `ppi.scala`, `ppi_.scala`, and `main.scala.in`.
...
If there are more `App`s' with agents that depend one to another, pass the `--interactive` option and all source files:
    ...
    ./examples $ ppi -akka --interactive ex1.scala ex2.scala
