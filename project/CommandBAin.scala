...
object CommandBAin {
  ...
  def bain = Command("bain") { state =>
    ...
    val opts = Map("-ce" -> (A ++ Seq(P, "-H0", "-T123456", "-Etrue", "-Cfalse", "-IsyncRate", "-Iprobability", "-IwhatIf", "-Sfalse")),
                   "-cef" -> (A ++ Seq(P, "-H0", "-T123456", "-Etrue", "-Cfalse", "-IsyncRate", "-Iprobability", "-IwhatIf", "-Sfalse")),
                   "-zio" -> (A ++ Seq(P, "-H0", "-T123456", "-Etrue", "-Cfalse", "-IsyncRate", "-Iprobability", "-IwhatIf", "-Sfalse")),
                   "-ziof" -> (A ++ Seq(P, "-H0", "-T123456", "-Etrue", "-Cfalse", "-IsyncRate", "-Iprobability", "-IwhatIf", "-Sfalse")),
                   "-fs2" -> (Seq(P, "-H0", "-T123456", "-Etrue", "-Cfalse", "-IsyncRate", "-Iprobability", "-IwhatIf", "-Sfalse") ++ F),
                   "-zs" -> Seq(P, "-H0", "-T123456", "-Etrue", "-Cfalse", "-IsyncRate", "-Iprobability", "-IwhatIf", "-Sfalse")
               )

    def suggestions(args: Seq[String]): Seq[String] =
      args.flatMap {
        case it if it.startsWith("-") && args.size == 1 =>
          Seq("-ce", "-cef", "-zio", "-ziof", "-fs2", "-zs").filter(_.startsWith(it.toLowerCase))
        case it =>
