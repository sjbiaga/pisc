...
object CommandSPin {
  ...
  def spin = Command("spin") { state =>
    ...
    val opts = Map("-ce" -> Nil, "-cef" -> Nil, "-zio" -> Nil, "-fs2" -> Seq("cats.effect.IO", "zio.Task").map("-F" + _), "-zs" -> Nil, "-kk" -> (0 to 1).map("-O" + _))

    def suggestions(args: Seq[String]): Seq[String] =
      args.flatMap {
        case it if it.startsWith("-") && args.size == 1 =>
          Seq("-ce", "-cef", "-zio", "-kk", "-fs2", "-zs").filter(_.startsWith(it.toLowerCase))
        case it =>
