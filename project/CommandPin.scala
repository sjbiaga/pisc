...
object CommandPin {
  ...
  def pin = Command("pin") { state =>
    ...
    val opts = Map("-ce" -> Nil, "-zio" -> Nil, "-fs2" -> Seq("cats.effect.IO", "zio.Task").map("-F" + _), "-monix" -> Seq("cats.effect.IO", "monix.eval.Task").map("-F" + _), "-zs" -> Nil, "-kk" -> (0 to 2).map("-O" + _))

    def suggestions(args: Seq[String]): Seq[String] =
      args.flatMap {
        case it if it.startsWith("-") && args.size == 1 =>
          Seq("-ce", "-zio", "-kk", "-fs2", "-monix", "-zs").filter(_.startsWith(it.toLowerCase))
        case it =>
