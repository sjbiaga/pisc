Stochastic Pi-calculus with (Biological) Transactions in SCala aka sPIXC ala RISC
=================================================================================
...
discard actions.
Another [branch](https://github.com/sjbiaga/pisc/tree/stochastic-bio-transactions)
...
The BNF formal grammar for prefixes is the following.
    ...
    PREFIXES       ::= { PREFIX }
    PREFIX         ::= "ν" "(" NAMES ")"
                     | "start" "(" trans ")" [ @ RATE ] INSTANTIATION "."
                     | "end" "(" trans ")" [ @ RATE ] "."
                     | μ "."
    μ              ::= "τ" [ @ RATE ] [ EXPRESSION ]
...
used. The traces' output is a `.csv` file with the following columns:
    ...
    number,start,end,name,polarity,key,replication,label,rate,delay,duration,agent,filename
...
originating _agent name_. The final thirteenth column is a _filename_ (with
...
To get the intermediary `in/ex.scala.in` file, execute the `run` command in the `sbt` shell:
    ...
    sbt:Stochastic π-Calculus[transactions]2Scala> run ex

where `example/pixc/ex.pixc` contains the stochastic π-calculus source (equations binding
...
- [π-calculus with biological transactions async](https://github.com/sjbiaga/pisc/tree/transactions-async)
...
- [Stochastic π-calculus with biological transactions async](https://github.com/sjbiaga/pisc/tree/stochastic-bio-transactions-async) using supervisor/`IO.canceled`
...
- [Stochastic π-calculus with biological transactions async](https://github.com/sjbiaga/pisc/tree/stochastic-bio-transactions-flatMap-async) with `flatMap`s/`null` comparison
